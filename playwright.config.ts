...
import { WaitForLoadStateOptions } from './src/finstreet/setup/optional-parameter-types';
import {
  ACTION_TIMEOUT,
  EXPECT_TIMEOUT,
  NAVIGATION_TIMEOUT,
  TEST_TIMEOUT,
} from './src/finstreet/utils/timeout-constants';
dotenv.config({ path: '.env' });

const BASE_URL = process.env.URL || 'https://qa-accounts.thefinstreet.co.uk';
...
export default defineConfig({
   ...
   */
  reporter: [['./src/finstreet/setup/custom-logger.ts'], ['html', { open: 'never' }], ['dot']],
  /**
   * Shared settings for all the projects below.
   * See https://playwright.dev/docs/api/class-testoptions
   */
  globalSetup: require.resolve('./src/finstreet/setup/global-setup.ts'),
  globalTeardown: require.resolve('./src/finstreet/setup/global-teardown.ts'),
